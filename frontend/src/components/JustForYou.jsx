import {PhilippinePeso, Star,StarHalf} from 'lucide-react'
...
export default function JustForYou() {
  ...
  <div>
    ...
    {/* Sale Products */}
    <div className='grid  md:grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-3 mt-5 max-w-6xl overflow-y-hidden  hide-Scrollba overflow-x-hidden' >
...
function generateStar(rating) {
  ...
  const hasHalfStar = Boolean(rating%1);
  const emptyStar = maxStars - fullStar - (hasHalfStar ? 1 : 0);
  const stars = []

  for (let i = 0; i < fullStar; i++) {
    stars.push(<Star size={18} color='orange' fill='orange' key={i} />)
  }

  if (hasHalfStar) {
    stars.push(<StarHalf size={18} color='orange' fill='orange' key={fullStar} stroke='orange' />)
  }
  for(let i = 0; i < emptyStar; i++) {
    stars.push(<Star size={18} color='gray' key={'empty' + i} />)
  }
